...
def mainFunction(product):
    ...
    for site in sites:
        ...
        for i in range(len(reviewsHeading)):
            ...
            review['upvotes'] = [ reviewsUpvotes[i].split(' ')[0], reviewsUpvotes[i].split(' ')[2]]
            review['stars'] = reviewStars[i].split(' ')[0]
            reviewTextSplit = reviewsText[i].split('.')
            finalText = ''
            for jj in reviewTextSplit:
                if(jj.find('Amazon') == -1 and jj.find('amazon') == -1 and jj.find('delivered') == -1  and jj.find('delivery') == -1):
                    finalText += jj
                    finalText += '.'
